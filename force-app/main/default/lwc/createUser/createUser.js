...
export default class CreateUser extends LightningElement {
  ...
  updateNickName() {
    const firstName = this.template.querySelector(
    ...
    if (input.reportValidity()) {
      ...
      if (value === undefined) {
        ...
        input.setCustomValidity(
          "The password is the wrong length (must be >= 7): " + value.length
        );
      } else if (value.length > 15) {
        input.setCustomValidity(
          "The password is the wrong length (must be <= 15): " + value.length
        );
